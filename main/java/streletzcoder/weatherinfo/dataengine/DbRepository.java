...
public class DbRepository {
    ...
    /**
     * Название страны для её отбражения в интерфейсе
     * @return
     */
    public String getSelectedCountryName() {
        ArrayList<String> list = getCountryData("_id='" + String.valueOf(getSelectedCountryId()) + "'", CountryFields.COUNTRY);
        if (list.size() != 0) {
            //Если для выбранной страны есть выбранный город, возвращаем его
            return list.get(0);
        } else {
            //Иначе возвращаем первый город данной страны из общего списка
            list = getCountryData(null, CountryFields.COUNTRY);
            return list.get(0);
        }
    }
    ...
    public void setSelectedCountry(String countryName) {
        setSelectedCountry(getCountryId(countryName));
    }
    ...
    public void setSelectedCountry(int countryId) {
        db.execSQL("UPDATE CountrySelected SET CountryId=" + countryId);
    }

    /**
     * Получаем id выбранного города по его названию
     *
     * @param cityName Название города
     * @return
     */
    ...
    /**
     * Получаем id выбанной страны по её названию
     * @param countryName Название страны
     * @return
     */
    private int getCountryId(String countryName) {
        ArrayList<String> list = new ArrayList<String>();
        String whereArgs[] = new String[1];
        whereArgs[0] = countryName;
        Cursor cursor = db.query("Country", null, "Country=?", whereArgs, null, null, null);
        if ((cursor != null) && (cursor.getCount() > 0)) {
            cursor.moveToFirst();
            do {
                list.add(cursor.getString(0));
